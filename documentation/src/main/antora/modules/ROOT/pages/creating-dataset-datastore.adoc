...
<<<<<<< component-runtime-1.30.0-branch
=======
    ...
    public CloseConnectionObject closeConn() {
        return new CloseConnectionObject() {
            ...
            public boolean close() throws ComponentException {
                Object connection = this.getRuntimeContext().getConnection();
                //do close action
                return true;
            }
...
Then the runtime mapper and processor only need to extends RuntimeContextInjector and can get the connection like this:
...
----
>>>>>>> fdd15ad use annotation to replace interface for auto inject
